...
const lockPeriods = poolsConfig.map((poolConfig) => {
  return {
...
export const fetchPoolsTotalStaking = async () => {
  ...
  console.log('testcode', poolsTotalStaked)
...
export const fetchPoolsLockPeriod = async () => {
  console.log('lockPeriods', lockPeriods)
  const poolsLockPeriod = await multicall(sousChefABI, lockPeriods)
  console.log('poolsLockPeriod', poolsLockPeriod)
  return poolsConfig.map((p, index) => ({
    sousId: p.sousId,
    lockPeriod: new BigNumber(poolsLockPeriod[index]).toJSON(),
  }))
}
